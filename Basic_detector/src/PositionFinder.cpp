...
G4bool PositionFinder::ProcessHits(G4Step* step, G4TouchableHistory*)
{
    ...
    if (step->GetTrack()->GetParticleDefinition()->GetPDGCharge() != 0.0)
    {
        ...
        analysisManager->FillNtupleDColumn(m_ID, 1, energy);
        analysisManager->FillNtupleDColumn(m_ID, 2, hitPosition.x());
        analysisManager->FillNtupleDColumn(m_ID, 3, hitPosition.y());
        analysisManager->FillNtupleDColumn(m_ID, 4, hitPosition.z());
        analysisManager->FillNtupleDColumn(m_ID, 5, hitMomentum.x());
        analysisManager->FillNtupleDColumn(m_ID, 6, hitMomentum.y());
        analysisManager->FillNtupleDColumn(m_ID, 7, hitMomentum.z());
